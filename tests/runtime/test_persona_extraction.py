...
async def test_extract_uses_main_role_by_default():
...
async def test_extract_normalises_date_to_python_date():
    ...
    out = await extract_persona_facts_and_blocks(llm=llm, context_text="ctx")
...
def test_format_events_thoughts_with_objects_and_tuples_equivalent():
    ...
    text_obj = format_events_thoughts_as_context(events=events_obj, thoughts=["慢热"])
    text_tup = format_events_thoughts_as_context(events=events_tup, thoughts=["慢热"])
