...
**Channel 是"哑"的 I/O 适配器。** 一个 channel 独占一条外部传输——Web UI、Discord、iMessage、WeChat——它的唯一职责是把文字在那条传输和 daemon 其余部分之间来回搬运。Channel 不调 LLM、不读记忆、不决定 persona 说什么、也不保存 persona 状态。所有"思考"发生在上一层：`runtime` + `memory` + prompt 组装器。如果一个 channel 实现开始想缓存 core block 或跑一次 retrieval 查询,这个设计就错了。
...
**`OutgoingMessage`** —— runtime 交给 `channel.send()` 的东西。只装一条哑 I/O 适配器所需要的东西:`content`、可选的 `in_reply_to_turn_id`、一个区分普通 `"reply"` 和自主 `"proactive"` 推送的 `kind`,以及一个 `delivery` 字段(当前代码库里是 `"text"` 或 `"voice_neutral"`),告诉 channel 应该怎么把消息物理递送出去。Persona 状态、episodic state、retrieval 结果都**不在**这里——它们早就被消费掉,变成了 `content`。
...
`routes/admin/` 下的 49 个 admin 路由处理器按域拆分,而不是堆在一个 2700 行的文件里。`routes/admin/__init__.py` 是组装入口——构造共享的 `APIRouter`,然后把注册委托给每个域的一个 `register_*_routes(router, *, …)` 函数:`core`(3 条 —— daemon state、reset、用户时区)、`config`(3 条 —— config GET/PATCH、channels PATCH)、`voice`(6 条 —— sample CRUD、克隆向导、activate)、`diagnostics`(8 条 —— cost、失败 session、slow-tick 转录、turn、consolidate trace)、`persona`(11 条 —— persona CRUD、头像、style、voice-toggle、facts、extract、bootstrap)、`memory`(18 条 —— events、thoughts、timeline、search、删除、trace、entities)。共享 helper 和 Pydantic 请求模型分别在 `routes/admin/helpers.py` 和 `routes/admin/models.py`。新增 admin 路由就是"打开对应域的文件加 handler";跨域的 endpoint(目前没有)会落在 `core.py`。

#### Memory Timeline 面板

Chat 页面右侧多一栏 **Memory Timeline**,默认展开,折叠到 40 px 竖向 tab,在 1200 px 以下变成抽屉式 overlay。它是 SSE-driven 的"persona 记忆生长"实时视图——每收到一条新 L3 event / L4 thought / L5 entity / L6 mood / session-close,timeline 顶部就插一行,反时序排列,每行 `icon + 相对时间("刚刚" / "半小时前") + 1 行摘要`,点开看详情。`...` 菜单提供按 kind 过滤(只看 event / 只看 thought 之类)和"清本地缓存"(强制再走一次 backfill)。

后端有两条数据通路。冷启:`GET /api/admin/memory/timeline?since=<iso>&limit=50` 一次性 backfill 最近若干条记忆事件,反时序合并 L3 / L4 / L5 / L6 / session-close 五种类型,server-side 过滤 uncertain entity,`updated_at > created_at` 的 entity description 编辑也会作为"description"事件出现在 backfill 里。热更:订阅前端 SSE 流上的 6 条 `memory.*` / `chat.*` topic(下表),命中即在 timeline 顶部 prepend 一行,客户端去重 + 排序 + cap 200 条。
...
useMemoryTimeline()          src/echovessel/channels/web/frontend/src/screens/Chat/timeline/
  ...
  └─ live subscription      6 SSE topics (见下面 SSE 目录)
```

filter 集合和 panel 折叠状态都持久化在 `localStorage`(key:`echovessel.timeline.filters` / `echovessel.timeline.collapsed`),新窗口接着用。

#### SSE topic 目录(Web channel 订阅的全集)

Web channel 是当前唯一暴露 `push_sse` 的 channel,既消费自己 turn 流的 `chat.message.*`,也消费 runtime memory observer 广播的 memory / 跨 channel 共享 topic。完整 topic 表:

| Topic | 来源 | Payload 关键字段 | 消费者 |
|---|---|---|---|
| `chat.message.user_appended` | web channel 自身 turn 流 | `turn_id` · `messages` | chat 主气泡列 |
| `chat.message.typing_started` | web channel 在 LLM stream 开始时推 | `turn_id` | chat 主气泡列(渲染"正在输入...") |
| `chat.message.done` | runtime broadcaster mirror 任意 channel turn 完成 | `turn_id` · `source_channel_id` · `content` | chat 主气泡列(跨 channel 实时 mirror) |
| `chat.message.voice_ready` | voice 服务异步合成完成 | `turn_id` · `audio_url` | chat 气泡内的播放控件 |
| `chat.session.boundary` | runtime memory observer | `closed_session_id` · `new_session_id` · `events_count` · `thoughts_count` | timeline 渲 session-close summary 行 |
| `chat.mood.update` | runtime memory observer | `persona_id` · `user_id` · `mood_summary` | timeline 渲 mood shift 行 |
| `chat.settings.updated` | runtime voice / admin runtime 配置变更 | `voice_enabled` 等 | settings UI |
| `memory.event.created` | runtime memory observer | `event_id` · `description` · `emotional_impact` · `session_id` | timeline event 行 |
| `memory.thought.created` | runtime memory observer | `thought_id` · `type` · `subject` · `description` · `source` · `filling_event_ids` | timeline thought / intention / expectation 行 |
| `memory.entity.confirmed` | runtime memory observer | `entity_id` · `canonical_name` · `kind` · `merge_status` | timeline entity 行(observer 已过滤 uncertain) |
| `memory.entity.description_updated` | runtime memory observer | `entity_id` · `canonical_name` · `description` · `source` | timeline entity description 行 |

`memory.*` 四条由 `RuntimeMemoryObserver` 直接驱动,详见 `runtime.md § Cross-Channel SSE topic 目录`。`chat.session.boundary` 由 close / open 两个 hook 共用——一边的 id 字段为 null;close 边附 `events_count` 和 `thoughts_count`。Discord / iMessage 等非 web channel 不暴露 `push_sse`,observer 自然 no-op 跳过——它们仍然只是把自己的 turn 流原样喂进 runtime 即可,跨 channel 的实时 mirror 由 broadcaster 帮它们打到 web 上。

### 跨 channel 统一 timeline(runtime 级 SSE broadcaster)

EchoVessel 的架构承诺是"一个 persona 跨所有 channel"。Memory 层一直在存储层保证这一点(iron rule D4:retrieval 永不按 `channel_id` 过滤),live view 也是统一的:

- `SSEBroadcaster` 由 runtime 持有,不属于任何单个 channel。
- 每个 channel 的 turn 事件(`chat.message.user_appended` / `chat.message.typing_started` / `chat.message.done` / `chat.message.voice_ready`)都经 broadcaster mirror · 带 `source_channel_id` 字段标记 turn 来源。`chat.message.typing_started` 在每次 turn LLM stream 开始前推一次——浏览器在收到 `chat.message.done` 前渲染一个"正在输入..."气泡。
- 同一份 SSE 流也承载 `RuntimeMemoryObserver` 广播的 memory 写入事件(`memory.event.created` / `memory.thought.created` / `memory.entity.confirmed` / `memory.entity.description_updated`)和跨 channel 共享的 `chat.session.boundary` / `chat.mood.update`。Web chat 的 Memory Timeline 面板就是这条流的消费者,也因此一条来自 Discord DM 的对话产生的记忆,实时出现在 Web tab 的 timeline 上,跟它消息出现在主气泡列是同一份 SSE。完整 topic 表见上面 [SSE topic 目录](#sse-topic-目录web-channel-订阅的全集)。
...
- 配对的 `GET /api/chat/history?limit=50&before=<turn_id>` 端点返回跨 channel 历史(仍然 D4-不过滤)· 新浏览器 session 会把过去 Discord 对话和 Web 对话一起回填。Memory Timeline 走另一条 backfill: `GET /api/admin/memory/timeline?since=<iso>&limit=50`,返回 L3/L4/L5/L6 跨类合并的记忆事件——同样 D4-不过滤。
...
Channel Protocol 的权威源头看 `src/echovessel/channels/base.py`。Registry 和 dispatch 管道看 `src/echovessel/runtime/channel_registry.py` 和 `src/echovessel/runtime/turn/dispatcher.py`。Turn pipeline 本身——memory 检索和 LLM 组装在那里强制执行"不泄漏传输"的规则——住在 `src/echovessel/runtime/turn/coordinator.py`。
...
iMessage channel 让 persona 通过 Apple Messages.app 接收 allowlist 里联系人的直接消息。持久化 persona、记忆、episodic state 都和 Web、Discord 共用 —— 唯一的区别是传输层。

我们**不自己实现** iMessage 协议。Apple 的 IDS / APNs 服务器只接受真 Apple 设备的签名,所以所有非官方项目最终都走同一条路:起一个 macOS 原生的小 helper 替你讲 iMessage。EchoVessel 选的是 [`steipete/imsg`](https://github.com/steipete/imsg) —— 一个 Swift CLI,接收走 `~/Library/Messages/chat.db`、发送走 AppleScript 驱动 Messages.app,对外通过 stdio 上的 JSON-RPC 暴露。我们的 channel 代码 spawn 一个长驻的 `imsg rpc` 子进程,订阅它的 `message` 通知流,发送时调它的 `send` 方法。

### 前置条件

- **macOS**。daemon 必须跑在登录了 persona Apple ID 的那台 Mac 上。Linux / Windows 不行。
- **强烈建议给 persona 单独一个 Apple ID**。开一个新的 iCloud 邮箱(免费)做 persona 身份,和你自己的主账号分开。虽然单账号测试也能跑通,但每台登录了你主账号的设备都会看到"自己和自己发消息"这种错位,体验割裂。
- **Messages.app 登录了** persona 的 Apple ID,并后台运行。
...
两项权限都是授予"运行 EchoVessel 的那个进程上下文"(通常就是你用来启动 daemon 的终端 app)。权限对已经跑着的进程不生效,改完要重启终端。

- **Full Disk Access(完全磁盘访问)** —— `chat.db` 在受保护的 `~/Library/Messages/` 下。打开 **系统设置 → 隐私与安全性 → 完全磁盘访问权限**,点 `+`,加你的终端 app(Terminal.app / iTerm / Warp / ghostty / VS Code —— 哪个就加哪个)。加完**完全退出并重开终端**。
- **Automation(自动化)** —— 发送需要 AppleScript 驱动 Messages.app。打开 **系统设置 → 隐私与安全性 → 自动化 → *你的终端* → Messages**,勾上。(这一项要等第一次发送尝试后才会出现。)
...
如果探活失败,错误信息会直接告诉你哪一步挂了(imsg 没装 / FDA 没给 / imsg 崩了 等等),每一条失败都带一个可执行的提示。
...
[channels.imessage]
enabled = true
persona_apple_id = "anya-persona@icloud.com"     # 必填
...
`persona_apple_id` 是目的地过滤字段 —— 发给同一台 Mac 上其他任何 Apple ID 的消息,会在进 LLM 之前就被丢弃。当一个 Messages.app 同时服务 persona 和你本人账号时,这就是核心隔离机制。

`allowed_handles` 是可选的发件人白名单。为空表示"接受所有发给 `persona_apple_id` 的 1:1 消息"。支持电话号码(E.164 格式比如 `+14155551234`,或者搭配 `region` 的裸数字)和邮箱。

不想动 `config.toml` 也行:打开 Web UI 的 **Admin → Channels → iMessage**,直接在面板里开关通道,行内改 `persona_apple_id` / `allowed_handles`。保存会调 `PATCH /api/admin/channels`,把合并后的 TOML 写到磁盘,并弹一个"需要重启"的提示 —— iMessage 子进程是启动时绑定的,在线编辑要重启 daemon 才生效。

### 5. 启动 daemon

像往常一样 `echovessel run`。启动成功的日志长这样:
...
从别的手机或 Mac 给 `persona_apple_id` 发一条 iMessage,一个 debounce 窗口之后 persona 会在同一条会话里回复。

### 已知限制(MVP)

- **只处理 1:1 私信。**群聊消息在 inbound pipeline 第四步就被丢了。群聊要做 mention 识别和 chat-id 路由,留给后续迭代。
- **只支持纯文本。**附件、表情回应、已读回执、编辑、撤回这些都不处理/不发送。AppleScript 对它们的支持也不干净。
- **没有历史回填。**`imsg watch.subscribe` 是从"当前时间"开始推送 —— daemon 关机期间到达的消息**不会**回灌进 memory。iMessage 服务器侧仍然会把消息送到 Messages.app,只是不会进 persona 的记忆,除非 daemon 当时在线。
- **发送偶尔会双发**(AppleScript 的 race)。inbound 这边的重复会被 echo cache 吞掉;outbound 这边的重复是平台的已知怪癖。

### 故障排查

- **探活报 "Full Disk Access is missing"** —— 你加错终端 app 了,或者加完没有彻底重启终端。FDA 是按"进程上下文"授予的,fork 时继承,已经跑着的终端进程永远拿不到后来加的权限。
- **`imsg` 找不到** —— `brew install` 没有落到当前用户的 `PATH` 上。`which imsg` 验证一下;如果不在 PATH,`cli_path = "/absolute/path/to/imsg"` 直接指绝对路径。
- **inbound 消息被静默丢弃** —— 看日志 `imessage: drop reason=…`。reason 字段会告诉你是目的地过滤、白名单、echo cache、还是 rate limiter 触发的。
- **persona 发出去的消息显示成"你本人"发的** —— 你忘了把 persona 的 Apple ID 登录到 Messages.app,导致 AppleScript 从你主账号发出去了。在接收端查一下发件人名字。
- **两个 bot 陷入死循环对话** —— rate limiter 会在 60 秒内 5 次 drop 之后静默整条会话。如果还在持续刷,说明对面也有一个 AI 在回复,这个问题单边解决不了。
