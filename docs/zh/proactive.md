...
v0.4 在 memory 模块里实装了 slow_tick reflection phase（详见 [`memory.md` § Slow_tick consolidate phase](./memory.md#slow_tick-consolidate-phase-l4-forward-looking--plan-7)）。slow_tick 在 session 之间跑,产 `type='thought'` 节点和 `type='expectation'` 子类节点（带 `event_time_end` 作为 due_at）。expectation 表达的语义是"我预期 user 会在某个时间窗里做某件事"——这看起来天然就是 proactive 的触发源:expectation 临近 due_at 时由 proactive 主动起话头("你之前说今晚会更新一下,有进展吗？")。

**v1 没有把这条线接上**——proactive 当前只读 L3 events / L4 thoughts / RecallMessages,不读 expectation 节点,不会因为 expectation 即将到期而 fire 主动消息。理由是:
...
- expectation 的 due_at 精度只到 day（plan §7）,proactive tick 是 60s——粗细对不上;接的话还得引入 day-bucket 节流逻辑,工作量不在这次 ship 范围。
