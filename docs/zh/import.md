...
- 抽取 prompt 明确列举六个合法目标(`L1.persona_block` / `L1.self_block` / `L1.user_block` / `L1.relationship_block` / `L3.event` / `L4.thought`),其它任何写入目标都会被 JSON 校验器拒绝。
...
导入管线和 runtime 在五模块架构里是同层 sibling,而 layering 契约禁止 sibling 之间互相 import:`channels.web` 不能直接 import `import_.pipeline`。中介是 `src/echovessel/runtime/wiring/importer.py`,它暴露任何调用方都需要的四个方法:
...
以上所有内容的权威来源是 `src/echovessel/import_/` 下的文件和 `src/echovessel/runtime/wiring/importer.py`;代码和本文档持有相同的不变量,当两者冲突时以代码为准。`tests/import_/` 下的测试套是可执行的规格,是验证任何扩展的最快方式。
