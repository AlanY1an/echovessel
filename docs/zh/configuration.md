...
`users` 表里每行一份用户档案,带 `display_name` 和 `timezone`。`[users]` config section 不直接定义 user(那是 admin API 的事),只承载与 user 行相关的全局默认。

| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `timezone` | `null`(无全局默认)| IANA 时区字符串(如 `"Asia/Taipei"` / `"America/New_York"`)。每行 `users.timezone` 优先;空时由 web channel 首次连接时浏览器 `Intl.DateTimeFormat().resolvedOptions().timeZone` 自动写入,owner 可在 admin UI 覆盖。`assemble_turn` 用它把 `IncomingMessage.received_at` 渲到 system prompt `# Right now` 第一行。 |

设计意图:user 时区**不是** config 静态项,因为同一个 daemon 可能服务多个 user(MVP 是单 user,但 schema 留了)。把它落在 `users` 行而不是 `[users]` 全局,免得日后扩展时把所有用户绑死在一个时区上。

## `[slow_tick]`

slow_tick reflection phase 的开关和 quota。完整设计见 [`memory.md`](./memory.md#slow_tick-consolidate-phase-l4-forward-looking--plan-7) 的 slow_tick 段。所有数值都有保守默认,适合在不会失控的前提下开机就跑;true believer 把 `enabled` 翻 true 以前先看 admin 的 transcript 标签页确认 cycle 走的是合理 prompt。

| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `enabled` | `true` | 全局 kill switch。false 时 G phase 直接 return,worker 跳过 slow_cycle 调用,proactive 路径不受影响。 |
| `cool_down_minutes` | `30` | 同一 persona 两次 slow_cycle 之间的最小间隔。本 session 出现 SHOCK(\|impact\| ≥ 8)时这个 cool-down 被跳过,允许立即 fire 一次。 |
| `daily_cap` | `36` | 每 24h 滚动窗口内最大 cycle 数。粗粒度 rate-limit,防止极端情绪密集的一天把 LLM 账单推到失控。 |
| `daily_input_token_budget` | `150_000` | 每 24h 跨所有 cycle 的累计 input token 上限。配合 daily_cap 形成两道闸。 |
| `daily_output_token_budget` | `30_000` | 同上,output token 一侧。 |
| `transcript_retention_days` | `14` | `develop-docs/slow_tick_transcripts/` 下 cycle JSON 保留多久,过期由 worker 自己 prune。设为 0 关闭 retention(transcript 一直留着,适合 debug 期)。 |
