...
The 49 admin route handlers under `routes/admin/` are split by domain rather than co-located in a single 2 700-line file. `routes/admin/__init__.py` is the composition entry — it builds the shared `APIRouter` and delegates to one `register_*_routes(router, *, …)` function per domain: `core` (3 routes — daemon state, reset, user timezone), `config` (3 — config GET/PATCH, channels PATCH), `voice` (6 — sample CRUD, clone wizard, activate), `diagnostics` (8 — cost, failed sessions, slow-tick transcripts, turns, consolidate trace), `persona` (11 — persona CRUD, avatar, style, voice-toggle, facts, extract, bootstrap), and `memory` (18 — events, thoughts, timeline, search, deletes, traces, entities). Shared helpers + Pydantic request models live in `routes/admin/helpers.py` and `routes/admin/models.py`. Adding a new admin route is "open the matching domain file and add the handler"; cross-domain endpoints (none today) would land in `core.py`.
